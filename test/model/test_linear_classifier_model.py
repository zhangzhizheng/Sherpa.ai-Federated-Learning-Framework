...
def test_linear_classifier_model_wrong_initialization():
    ...
    for init_ in zip(n_features, classes):
        with pytest.raises(AssertionError):
            LinearClassifierModel(n_features=init_[0], classes=init_[1])
...
def test_logistic_regression_model_train_evaluate():
    ...
    lgr_ref = LogisticRegression(max_iter=500).fit(train_data, train_labels)
...
def test_linear_svc_model_train_evaluate():
    data, labels = load_iris(return_X_y=True)
    randomize = np.arange(len(labels))
    np.random.shuffle(randomize)
    data = data[randomize, ]
    ...
    model = LinearSVC(random_state=123, max_iter=5000)
    ...
    svc_ref = LinearSVC(random_state=123, max_iter=5000).fit(train_data, train_labels)
