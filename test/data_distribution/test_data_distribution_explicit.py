...
from shfl.private import LabeledData, UnprotectedAccess
...
def test_make_data_federated():
    ...
    percent = 100
    federated_data, federated_label, nodes = data_distribution.make_data_federated(train_data,
                                                                                   train_label,
                                                                                   percent)
    ...
    for data in all_data:
        idx.append(np.where((data == np.delete(train_data, 0, 1)).all(axis=1))[0][0])

    assert all_data.shape[0] == int(percent * train_data.shape[0] / 100)
    assert len(federated_data) == len(np.unique(train_data[:, 0]))
    assert (np.sort(all_data.ravel()) == np.sort(np.delete(train_data, 0, 1)[idx, ].ravel())).all()
...
def test_make_data_federated_pandas():
    ...
    assert all_data.shape[0] == int(percent * train_data.drop(train_data.columns[0], axis=1).shape[0] / 100)
    assert len(federated_data) == len(np.unique(train_data.iloc[:, 0]))
    pd.testing.assert_frame_equal(all_data, train_data.drop(train_data.columns[0], axis=1).iloc[all_data.index.values])
...
def test_get_data_federated():
    ...
    percent = 100
    federated_train_data, federated_train_label, nodes = data_distribution.make_data_federated(train_data,
                                                                                               train_label,
                                                                                               percent)
    ...
    for node in range(len(nodes)):
        assert federated_data[node].federated_data_identifier == nodes[node]
